...
router.post('/signup', async (req, res) => {
  try {
    ...
    return res.status(201).json({ ok: true, message: 'Signup successful, please verify your email', emailSent });
  } catch (err) {
...
router.get('/google/callback', (req, res, next) => {
  ...
  try {
    ...
    const token = signToken(user);
    setAuthCookie(res, token);
    const redirectUrl = `${clientURL.replace(/\/+$/, '')}/?login=google`;
    return res.redirect(302, redirectUrl);
  } catch (err) {
...
router.get('/github/callback', (req, res, next) => {
  ...
  try {
    ...
    const token = signToken(user);
    setAuthCookie(res, token);
    const redirectUrl = `${clientURL.replace(/\/+$/, '')}/?login=github`;
    return res.redirect(302, redirectUrl);
  } catch (err) {
